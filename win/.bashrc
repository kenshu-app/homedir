...
alias gconi='codei ~/.gitconfig' # 設定ファイルをVSCodeで開く
...
alias gcp='git cherry-pick' # チェリーピック
...
alias gpf='git push -f origin main' # orginリポジトリに強制的にアップ
...
alias gig='code .gitignore' # addの対象除外ファイルをVSCodeで開く
alias gign="touch .gitignore | echo -e 'node_modules\nvendor\n.env\nimages' > .gitignore" # node_modulesをaddの除外対象に設定
...
# 自身のディレクトリを初期化してローカルとリモートのリポジトリを作成と同期
# 任意のコミットメッセージを元にREADME.mdを作成
function ghrao(){ ABSPATH=`pwd` && REPONAME=`basename $ABSPATH` && echo "#" $REPONAME > README.md && gi && gbmain && gacm "first commit" && gh repo create $REPONAME --public --source=. --remote=origin && gpom && ls -la && cat README.md && ghb && glo; }
...
alias ghlab='gh label clone kenshu-app/label' # ラベルテンプレの読み込み
...
# mysqlのログイン
alias ms='cd ../../mysql/bin'
alias msr='./mysql -uroot'
alias msrr='./mysql -uroot -proot'
alias mslogin='cd ../../mysql/bin && ./mysql -uroot'
